...
WITH transaction_metrics AS (
  SELECT 
    shop_id,
    COUNT(*) AS transaction_count,
    COUNTIF(status = 'declined') / NULLIF(COUNT(*), 0) AS decline_rate,
    SUM(amount) AS total_amount,
    SUM(amount) / NULLIF(COUNT(*), 0) AS avg_transaction_amount,
...
WITH daily_transactions AS (
  ...
  FROM `shopify-dw.money_products.order_transactions_payments_summary`
  WHERE created_at >= DATE_TRUNC(DATE_SUB(CURRENT_DATE(), INTERVAL 1 MONTH), MONTH)
    AND status = 'success'
  GROUP BY DATE(created_at)
)
...
shop_risk AS (
  SELECT
    ...
    DATE_DIFF(a.last_transaction, a.first_transaction, DAY) AS days_active
...
monthly_activity AS (
  ...
  WHERE created_at >= DATE_SUB(CURRENT_DATE(), INTERVAL 12 MONTH)
    AND status = 'success'
  GROUP BY shop_id, DATE_TRUNC(DATE(created_at), MONTH)
),
...
6. Include the "Last Validated" date in your query
